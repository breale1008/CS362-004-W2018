...
// testing the council room card (+4 cards, +1 buy, +1 card for all other players)
...
void randomtestcardcouncilroom(struct gameState *G, int verboseMode) {
	...
	if (numHandCards(G) > 0) {
		...
		// forrce hand to contain council room
		handPos = rand() % numHandCards(G);
		G->hand[G->whoseTurn][handPos] = council_room;

		// >>>> do the council room card's effects <<<<
		int n = cardEffect(council_room, 0, 0, 0, G, handPos, &bonus);

		// test +3 cards
		testAssert((numHandCards(G) == Gpre.handCount[wTurn] + 3), "incorrect hand count", 0, verboseMode);
...
int main() {
	...
	for (n = 0; n < 1000; n++) {
		...
		randomtestcardcouncilroom(&G, 0);
		//randomtestcardcouncilroom(&G, 1);
		//randomtestcardcouncilroom(&G, 1);
	}
